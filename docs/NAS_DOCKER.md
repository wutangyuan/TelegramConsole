...
- 多账户添加、同时运行、停止和移除。
- 管理首页统一显示进程内存、数据目录、磁盘 I/O 和 Telegram 上下行流量。
- 消息终端最多保留最近 300 条，按时间和消息 ID 稳定合并；无数据变化时不重绘，阅读历史消息时不会被轮询强制滚到底部。
- 消息支持引用回复、编辑、撤回和按当前会话允许范围添加表情回应；编辑及回应更新按消息 ID 覆盖原记录。
