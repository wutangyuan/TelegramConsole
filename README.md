...
- WPF 使用唯一管理中心统一维护多账户、设备资源、全局异常和运行日志；各账号工作区只承载聊天、监控及账号自动化业务，后台自动启动账号不会弹出或闪烁工作区窗口。
- 登录成功后锁定登录入口，仅显示当前账号和在线/连接异常状态，避免误点重复登录。
- 私聊、群聊和独立 TUI 风格聊天控制台。
- 聊天终端支持“控制台 / 可视化 / 分屏”三种显示模式；可视化模式参考 Unigram 的消息气泡结构，复用同一条历史与实时消息流，不改变登录、监控、发送和定时任务流程。
- 可视化消息支持引用关系、自己/他人/@我的样式、撤回缓存标识，以及图片、视频、动图、语音、音频、文件、贴纸、投票、位置、联系人和网页等媒体卡片；可下载媒体可直接打开。
- 可视化模式按需加载账号隔离的媒体缩略图，并显示日期分隔、转发来源、作者签名、编辑/置顶状态、浏览数、转发数和表情回应；正文链接可直接打开，引用块可跳转原消息。
- 可视化消息右键支持引用、编辑、撤回、转发、复制正文、复制链接和快速表情回应，全部复用 WTelegramClient 服务接口。
...
本项目使用 [GNU General Public License v3.0](LICENSE)。从本版本开始，衍生发布物也须遵守 GPL-3.0 的开源与分发要求。
